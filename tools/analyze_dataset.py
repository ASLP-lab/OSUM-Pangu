...
"""
Anaosum_echat2e Dataset, Duration/TextLength/Speed etc.

Usage:
. ./path.sh && python3 tools/anaosum_echat2e_dataset.py \
    --data_type "shard" \
    --data_list data/test/data.list \
    --output_dir exp/anaosum_echat2e_test \
    --num_thread 32
"""
...
def get_args():
    parser = argparse.ArgumentParser(description='Anaosum_echat2e dataset')
...
def anaosum_echat2e(datas, output_file, thread_id):
...
def main():
    ...
    with open(args.output_dir + "/anaosum_echat2e_result_brief", "w",
              encoding='utf8') as f:
        for i, (name, unit, avg,
            ...
            f.write("std {}: {:.3f}\n".format(name, std))
    os.system("cat {}".format(args.output_dir + "/anaosum_echat2e_result_brief"))

    datas.sort(key=lambda x: x['dur'])
    with open(args.output_dir + "/anaosum_echat2e_result", "w", encoding='utf8') as f:
        for data in datas:
