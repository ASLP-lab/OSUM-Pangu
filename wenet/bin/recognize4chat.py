...
def main():
    ...
    for mode in modes:
        ...
        file_name = os.path.join(dir_name, 'result.jsonl')
    ...
    with torch.cuda.amp.autocast(enabled=True,
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(test_data_loader):
                ...
                # res_text = model.infer_sample_for_text_task(wavs=feats, wavs_len=feats_lengths, prompt=prompt)
                try:
                ...
                for mode in modes:
                    line = json.dumps({"key": keys[0],"response": res_text[0]}, ensure_ascii=False)
