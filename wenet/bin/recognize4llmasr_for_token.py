...
def main():
    ...
    global_prompt_dict = utils_file.load_dict_from_yaml(test_conf.get('prompt_conf_path', 'conf/prompt_config.yaml'))
    ...
    with torch.cuda.amp.autocast(enabled=True,
        ...
        with torch.no_grad():
            ...
            for batch_idx, batch in enumerate(test_data_loader):
                ...
                speech_token = batch['speech_tokens'].to(device)
                utils_file.logging_limit_print(speech_token)
                ...
                utils_file.logging_limit_print(extro_info)
                if 'question' in extro_info[0]:
                ...
                # res_text = model.infer_for_text2token(wavs=feats, wavs_len=feats_lengths, prompt=prompt)
                # res_text = model.forward_tmp(batch, device)
                res_text = model.infer_sample(device=device, prompt=prompt, text=target,speech_token=speech_token)
                # res_text = model.infer_sample_teach_force(wavs=feats, wavs_len=feats_lengths, prompt=prompt, text=target,speech_token=speech_token)
                for mode in modes:
