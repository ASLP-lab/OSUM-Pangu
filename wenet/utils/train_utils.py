...
from wenet.utils.init_dataset import init_dataset, init_big_dataset
...
def add_dataset_args(parser):
    ...
    parser.add_argument('--train_data_s2t', required=True, help='train data file')
    parser.add_argument('--train_data_t2s', required=True, help='train data file')
    parser.add_argument('--train_data_s2s', required=True, help='train data file')
    parser.add_argument('--train_data_t2t', required=True, help='train data file')
...
def check_modify_and_save_config(args, configs, symbol_table):
    ...
    if configs.get("model_conf",{}).get("apply_non_blank_embedding", False):
...
def init_dataset_and_dataloader(args, configs, tokenizer, seed=777):
    ...
    train_dataset = init_big_dataset(dataset_type,
                                 args.data_type,
                                 args.train_data_s2t,
                                 args.train_data_t2s,
                                 args.train_data_s2s,
                                 ...
                                 True,
                                 split='train')
    tag = configs.get('init_infos',{}).get("tag", "init")
    train_dataset.set_epoch(configs.get('init_infos',{}).get('epoch', 0) + int("epoch_" in tag) - 1)
...
def batch_backward(model, scaler, info_dict):
    ...
    for loss_name, loss_value in info_dict['loss_dict'].items():
        if loss_value is not None and not isinstance(loss_value, str):
...
def log_per_step(writer, info_dict, timer: Optional[StepTimer] = None):
    ...
    if tag == "TRAIN" and rank == 0 and writer is not None:
        if (train_engine == "deepspeed" and is_gradient_accumulation_boundary
            ...
            for name, value in loss_dict.items():
                if name != 'loss' and value is not None and not isinstance(value, str):
    ...
    if (batch_idx + 1) % log_interval == 0:
        ...
        for name, value in loss_dict.items():
            ...
            elif name != 'loss' and isinstance(value, str):
